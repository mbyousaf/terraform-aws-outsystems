...
resource "aws_lb" "Outsystems-alb" {
  name               = "${var.environment}-euw1-outsystems-alb-01"
  internal           = "false"
...
resource "aws_db_instance" "mssql-rds" {
  identifier                = "${var.environment}-euw1-outsystems-os-mssql"
  ...
  option_group_name         = "${var.environment}-euw1-sqlserver-se-15"
...
resource "aws_db_subnet_group" "mssql" {
  name       = "${var.environment}-euw1-outsystems-subnet-group"
  subnet_ids = var.rds_private_subnets
}

# The Option Group
resource "aws_db_option_group" "optiongroup" {
  name                     = "${var.environment}-euw1-sqlserver-se-15"
